...
**無料（MIT）：** 個人、オープンソースプロジェクト、本番環境での評価を行うチーム — 期限なし。
**有料（BLA）：** チームが本番環境に商用リリースする際に必要です。

自由に評価してください。本番リリースの準備ができたら購入してください。

詳細は [LICENSING.md](LICENSING.md) をご覧ください。

**商用利用？** [ライセンス階層を見る](https://lateos.ai/npm-scan/licensing) — $799/年（小規模チーム）から $9,900/年（エンタープライズ）。ボリュームディスカウントあり。
