...
async function testIncidentA() {
  ...
  const hookFindings = await lifecycleHookScan(
    pkgJson,
    allFiles.filter((f) => f.path.endsWith('.js')),
    registryMeta,
    allFiles
  );
  ...
  const followthroughFindings = await hookFollowthroughScan(
    pkgJson,
    allFiles.filter((f) => f.path.endsWith('.js')),
    registryMeta,
    allFiles
  ...
  const binaryFindings = await binaryEmbedScan(
    pkgJson,
    allFiles.filter((f) => f.path.endsWith('.js')),
    registryMeta,
    allFiles
  ...
  const maintainerFindings = await maintainerCompromiseScan(
    pkgJson,
    allFiles.filter((f) => f.path.endsWith('.js')),
    registryMeta,
    allFiles
  ...
  const multistageFindings = await multistagePostinstallScan(
    pkgJson,
    allFiles.filter((f) => f.path.endsWith('.js')),
    registryMeta,
    allFiles
...
async function main() {
  ...
  console.log('\n\n=== SUMMARY ===');
  console.log(
    'Incident A: Jscrambler - 2/5 detectors fire (hook-followthrough, maintainer-compromise)'
  );
